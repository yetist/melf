...
static int create_section_file(Elf *elf, const char* section, const char* secfile);
static int update_section_file(Elf *elf, const char* section, const char* secfile);
static int create_section_string(Elf *elf, const char* section, const char* str);
static int update_section_string(Elf *elf, const char* section, const char* str);
static Elf_Scn* find_section(Elf *elf, const char* scname);
static int print_section (Elf *elf, Elf_Scn *scn);
static void print_header (Elf *elf);
...
int show_section(const char* elfname, const char* section)
{
	...
	if ((scn = find_section(elf, section)) != NULL) {
		print_section (elf, scn);
	}
	if (elf_end (elf) != 0) {
...
int set_section_file(const char* elfname, const char* section, const char* secfile)
{
	...
	if ((scn = find_section(elf, section)) == NULL) {
		create_section_file(elf, section, secfile);
	} else {
		update_section_file(elf, section, secfile);
	}
	if (elf_end (elf) != 0) {
		printf ("failure in elf_end: %s\n", elf_errmsg (-1));
	}
...
int set_section_string(const char* elfname, const char* section, const char* str)
{
	...
	if ((scn = find_section(elf, section)) == NULL) {
		create_section_string(elf, section, str);
	} else {
		update_section_string(elf, section, str);
	}
	if (elf_end (elf) != 0) {
		printf ("failure in elf_end: %s\n", elf_errmsg (-1));
	}
...
static int create_section_file(Elf *elf, const char* section, const char* secfile)
{
	...
	isoftse = ebl_strtabadd (shst, section, 0);
	...
	if ((data = elf_newdata (isoft_scn)) == NULL ) {
